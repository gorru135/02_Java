...
public class MemberService {
	...
	public MemberService() { // 기본생성자
		...
		memberArr[0] = new Member("user01", "pass01","홍길동",30,"경기도");
		memberArr[1] = new Member("user02", "pass02","서길동",31,"서울");
		memberArr[2] = new Member("user03", "pass03","남길동",34,"부산");
	}
	...
	public void displayMenu() {
		...
		do {
			...
			case 3 : System.out.println( userSearch()); break;
			case 4 : 
	...
	public int emptyIndex() {
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] == null) {
				return i; //현재 메서드를 종료하고 호출한곳으로 i값을 가지고 돌아감
	...
	public String signUp() {
		...
		int index = emptyIndex(); // memberArr배열에서 비어있는 인덱스를 반환받음(없다면 -1반환)
		
		if(index == -1) { //비어있는 index가 없는 경우 -> 회원 가입 불가 
		...
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] != null) { // 회원 정보가 있을 경우 
				...
				if( memberArr[i].getMemberId().equals(memberId) && 
					...
					// 3) 로그인 회원 정보 객체를참조하는 변수 loginMember에
