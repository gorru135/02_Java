...
public class ByteService {
	...
	/* **************************************************** */
	// IO 관련된 코드는 IOExceptoin이 발생될 가능성이 높다! 
	...
	public void fileByteOutput() {
		...
		try {
			...
			for(int i = 0; i < content.length(); i++) {
				char ch = content.charAt(i);// i번재 글자 반환 
				fos.write(ch); // 1byte 출력 스트림이 연결된 파일에 ch 쓰기 
			}*/
			
			// System.nanoTime() : 1970.0101 09:00:00 부터
			...
			fos.flush(); // 스트림 안에 남아있는 모든 데이터를 다 밀어냄
	...
	public void bufferdFileByteOutput() {
		 ...
		 */
	FileOutputStream fos = null;
	BufferedOutputStream bos = null; // 보조 스트림 생성 
		...
		try {
			...
			try {
				...
				// 보조스트림 close() 시
				// 보조스트림 생성에 사용된 기반 스트림도 같이 "closd"
	...
	/**
	 * bufferdFileByteInput 보조 스트림을 이용한 성능 향상 
	 */
